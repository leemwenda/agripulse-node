...
export function EmptyState({ icon, title, description, action }: {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 sm:py-16 text-center">
      <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-full bg-gray-100 flex items-center justify-center mb-2 sm:mb-4 text-gray-400">
        {icon}
      </div>
      <h3 className="text-sm sm:text-lg font-semibold text-gray-900 mb-1">{title}</h3>
      {description && <p className="text-xs sm:text-sm text-gray-500 max-w-sm mb-4 sm:mb-6">{description}</p>}
      {action}
    </div>
  );
}
