...
export default function CreateListing() {
  ...
  const [form, setForm] = useState({ animalId:'', askingPrice:'', description:'', location:'' });

  const counties = ['Nairobi','Kiambu','Nakuru','Meru','Nyandarua','Laikipia','Trans Nzoia','Uasin Gishu','Kericho','Bomet','Nyeri','Muranga','Kirinyaga','Embu','Other'];
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await api.post('/marketplace/listings', { ...form, animalId: parseInt(form.animalId), askingPrice: parseFloat(form.askingPrice) });
        ...
        {success ? (
          <div style={{ background:D.card, border:`1px solid ${D.border}`, borderRadius:16, padding:'40px 32px', textAlign:'center' }}>
            ...
            <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
              ...
              <button onClick={()=>{ setSuccess(''); setForm({ animalId:'', askingPrice:'', description:'', location:'' }); }} style={{ padding:'11px 24px', background:'transparent', border:`1px solid ${D.border}`, color:D.text2, borderRadius:10, fontWeight:700, cursor:'pointer' }}>List Another</button>
            </div>
          </div>
        ) : (
            ...
            <div style={{ background:D.card, border:`1px solid ${D.border}`, borderRadius:16, padding:'20px' }}>
              ...
              {animals.length === 0 ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {animals.map(a => (
                    <label key={a.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:11, border:`2px solid ${form.animalId===String(a.id)?'#15803d':D.border}`, background:form.animalId===String(a.id)?isDark?'rgba(21,128,61,.1)':'rgba(21,128,61,.05)':D.card, cursor:'pointer', transition:'all .15s' }}>
                      <input type="radio" name="animal" value={a.id} checked={form.animalId===String(a.id)} onChange={e=>setForm(p=>({...p,animalId:e.target.value}))} style={{ display:'none' }}/>
                      <div style={{ width:40, height:40, borderRadius:10, background:isDark?'#1c2128':'#f1f5f9', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, flexShrink:0 }}>🐄</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:14, fontWeight:700, color:D.text }}>{a.name}</div>
                        <div style={{ fontSize:12, color:D.text3 }}>{a.breed} · {a.category} · {a.tagNumber}</div>
                      </div>
                      {form.animalId===String(a.id) && <CheckCircle2 size={18} color="#15803d"/>}
                    </label>
                  ))}
                </div>
              )}
