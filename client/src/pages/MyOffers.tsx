...
import { ArrowLeft, TrendingDown, ShoppingBag, CheckCircle2, XCircle, RefreshCw } from 'lucide-react';
import api from '../lib/api';
import { useTheme } from '../context/ThemeContext';

function fmt(v: any) { return v ? `KSh ${Number(v).toLocaleString()}` : '—'; }

const BUY_NOW_NOTE = 'Buy Now — purchase at asking price';
...
export default function MyOffers() {
  ...
  async function withdraw(id: number) {
    try { await api.delete(`/market-offers/${id}`); setMsg('Cancelled.'); load(); }
  ...
  const isBuyNow = (o: any) => o.note === BUY_NOW_NOTE;
  const orders = offers.filter(isBuyNow);
  const negotiated = offers.filter((o: any) => !isBuyNow(o));

  function Card({ offer, isOrder }: { offer: any; isOrder: boolean }) {
        ...
        <div style={{ display: 'flex' }}>
          ...
          <div style={{ flex: 1, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 10, color: text3, textTransform: 'uppercase', letterSpacing: '.5px' }}>{isOrder ? 'Order Total' : 'Your Offer'}</div>
            ...
            {!isOrder && offer.note && <div style={{ fontSize: 12, color: text3, fontStyle: 'italic', marginBottom: 10 }}>"{offer.note}"</div>}
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              {!isOrder && offer.status === 'countered' && counter && (
              ...
              {offer.status === 'pending' && (
                <button onClick={() => withdraw(offer.id)}
                  ...
                  <XCircle size={12} /> {isOrder ? 'Cancel Order' : 'Withdraw'}
                </button>
              )}
            </div>
        ...
        {loading ? (
          ...
          <>
            {orders.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  <ShoppingBag size={16} color={text2} />
                  <h2 style={{ fontSize: 15, fontWeight: 800, color: text, margin: 0 }}>My Orders</h2>
                  <span style={{ fontSize: 12, color: text3 }}>Buy Now purchases — awaiting seller confirmation</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {orders.map((offer: any) => <Card key={offer.id} offer={offer} isOrder={true} />)}
                </div>
              </div>
            )}
            ...
            {negotiated.length > 0 && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  <TrendingDown size={16} color={text2} />
                  <h2 style={{ fontSize: 15, fontWeight: 800, color: text, margin: 0 }}>My Offers</h2>
                  <span style={{ fontSize: 12, color: text3 }}>Negotiated prices</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {negotiated.map((offer: any) => <Card key={offer.id} offer={offer} isOrder={false} />)}
                </div>
              </div>
            )}
          </>
