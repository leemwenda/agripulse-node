  ...
  const FormContent = () => isLogin ? (
    <>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: 'rgba(255,255,255,.4)', marginTop: 22 }}>
        ...
        <button type="button" onClick={()=>window.location.href='/api/auth/google?portal=vet'}
          style={{ height:48, background:'rgba(255,255,255,0.07)', border:'1.5px solid rgba(255,255,255,.15)', borderRadius:12, fontWeight:600, fontSize:14, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:10, color:'#fff', width:'100%' }}>
        ...
        <p>Don't have an account? <Link to="/vet/register" style={{ color: BLUE, fontWeight: 700, textDecoration: 'none' }}>Sign up here</Link>
      </p>
    </>
  ) : (
    <>
      ...
      <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        ...
        <div style={{ padding:'10px 14px', background:'rgba(13,148,136,.1)', border:'1px solid rgba(13,148,136,.25)', borderRadius:10, fontSize:12, color:'rgba(255,255,255,.6)', lineHeight:1.5 }}>
          <FileText size={18} /> Your account will be reviewed by an admin before activation. You'll receive an email once approved.
        </div>
