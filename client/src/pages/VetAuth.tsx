...
export default function VetAuth() {
  ...
  async function handleRegister(e: FormEvent) {
    ...
    setLoading(true);
    try {
      await api.post('/auth/register', { name: regForm.name, email: regForm.email, phone: regForm.phone, county: regForm.county, password: regForm.password, role: 'vet' });
      navigate('/vet/login');
    } catch (err: any) {
  ...
  const inp: any = { width: '100%', height: 48, padding: '0 14px', border: '1.5px solid rgba(255,255,255,.15)', borderRadius: 10, fontSize: 14, outline: 'none', background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(8px)', color: '#fff', boxSizing: 'border-box' };
